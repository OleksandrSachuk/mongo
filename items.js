...
function ItemDAO(database) {
  ...
  this.getNumItems = function (category, callback) {
    ...
    this.db.collection('item').count(query , function(err, count) {
      callback(count);
    });
  }
  ...
  this.getNumSearchItems = function (query, callback) {
    ...
    // callback(numItems);
    this.db.collection('item').count({$text: {$search: query}} , function(err, count) {
      callback(count);
    });
  }
  ...
  this.addReview = function (itemId, comment, name, stars, callback) {
    ...
    { _id: itemId },
    { $push: { reviews: reviewDoc }}, function (err, doc) {
        assert.equal(null, err);
      callback(doc);
      });
